...
class HomeActivity : AppCompatActivity() {
    ...
    private fun checkForEmptyField() {
        if (guest_name_edittext.text.isNullOrBlank() || room_number_edittext.text.isNullOrBlank() || room_price_edittext.text.isNullOrBlank()) {
            AlertDialog
                ...
                .setMessage("Non of the fields should be empty. Please fill in all the necessary information.")
    ...
    private fun insertGuest() {
        ...
        val roomPrice = room_price_edittext.text.toString().toInt()
        val guest = GuestEntity(guestName, roomNumber, roomPrice)
        hotelDatabase.hotelDAO().insertGuest(guest)
        Toast.makeText(this, "$guestName checked into $roomNumber", Toast.LENGTH_SHORT).show()
    }
}
